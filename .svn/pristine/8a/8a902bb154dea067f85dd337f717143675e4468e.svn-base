...
class Wechat
{
    ...
    public function __construct($options = array())
    {
        ...
        $this->AESKey = isset($options['encodingaeskey']) ? $options['encodingaeskey'] : '';
        $this->mch_id = isset($options['mchid']) ? $options['mchid'] : '';
        $this->payKey = isset($options['mkey']) ? $options['mkey'] : '';
    ...
    public function get_address_sign($infolist)
    {
        ...
        $jsApiObj["accessToken"] = $this->access_token;
        $jsApiObj["nonceStr"] = $infolist['nonceStr'];
        $jsApiObj["timeStamp"] = $infolist['timestamp'];;
        $jsApiObj["url"] = $infolist['url'];
        $jsApiObj["addrSign"] = $this->getSign_address($jsApiObj);
        return $jsApiObj;
    }

    //获取登录JS接口
    public function getSignPackage($appUrl = '')
    {
    ...
    /**
     * 检查用户签名信息     * @author 金哥 <admin@cnbbx.com>
     */
    ...
    public function group_edit($gid = '', $name = '')
    {
        ...
        $params = array('group' => array('id' => $gid, 'name' => $name));
        $params = $this->json_encode($params);
    ...
    public function user($openid = '')
    {
        ...
        $params = array('access_token' => $this->access_token, 'lang' => 'zh_CN', 'openid' => $openid);
    ...
    public function user_to_group($openid = '', $gid = '')
    {
        ...
        $params = array('openid' => $openid, 'to_groupid' => $gid);
        $params = json_encode($params);
    ...
    public function getOauthAccessToken()
    {
        ...
        $params = array('appid' => $this->appid, 'secret' => $this->secret, 'code' => $code, 'grant_type' => 'authorization_code');
    ...
    private function getJsApiTicket()
    {
        ...
        if ($data->expire_time < time()) {
            $params = array('access_token' => $this->access_token, 'type' => 'jsapi');
    ...
    {
        $arr = array("appid" => $this->appid, "redirect_uri" => 'http://wx.cnskl.com/' . $fun, "response_type" => 'code', "scope" => $scope, 'state' => $userid);
     ...
     */
    public function unifiedOrder($openid, $body, $orderId, $money, $notify_url = '', $extend = array(), $trade_type = 'JSAPI')
    {
            ...
            if ($data['return_code'] == 'SUCCESS') {
                if ($data['result_code'] == 'SUCCESS') {
                    if ($trade_type == 'NATIVE') {
                        ...
                        return $this->app_createPayParams($data['prepay_id']);
                    } else {
     ...
     * 将数组拼接成xml
     */
    private function myarraytoxml($data)
    {
        $xml = "<textarea><xml>";
     ...
     * 生成支付参数
     */
    private function app_createPayParams($prepay_id)
    {
        if (empty($prepay_id)) {
            $this->error = 'prepay_id参数错误';
            return false;
     ...
     * 发放红包
     */
    public function sendredpack($orderId, $re_openid, $total_fee, $refund_fee = '')
    {
        $params['wxappid'] = $this->appid;
        $params['mch_id'] = $this->mch_id;
        $params['mch_billno'] = $orderId;
        $params['nick_name'] = "艾美e族财富通";
        $params['send_name'] = "艾美e族财富通";
        $params['re_openid'] = $re_openid;
        $params['total_amount'] = $refund_fee;
        $params['min_value'] = $refund_fee;
        $params['max_value'] = $refund_fee;
        $params['total_num'] = 1;
        $params['act_name'] = "艾美e族财富通工资发放";
        $params['wishing'] = "您这段时间的推广获得了裂变工资。";
        $params['remark'] = "分销越多得越多，快来抢！";
        $params['client_ip'] = get_client_ip();
        $params['logo_imgurl'] = get_client_ip();
        $params['nonce_str'] = self::_getRandomStr();
        $params['sign'] = self::_getOrderMd5($params);
        $data = self::_array2Xml($params);
        $data = $this->http(self::SEND_REDPACK_URL, $data, 'POST', true);
        return self::parsePayRequest($data);
    }
    ...
    public function returnNotify($return_msg = true)
    {
        if ($return_msg == true) {
            ...
            $data = array('return_code' => 'FAIL', 'return_msg' => $return_msg);
        }
        echo self::_array2Xml($data);
        exit;
    }
     ...
     * 对支付回调接口返回成功通知
     */
    public function returnstr_Notify($return_msg = true)
    {
        if ($return_msg == true) {
            $data = array('return_code' => 'SUCCESS', 'return_msg' => 'OK');
        } else {
    ...
    private function _getOrderMd5($params)
    {
        ...
        //pre(urldecode(http_build_query($params)));
     ...
     * 本地MD5签名
     */
    private function _getOrderMd5s($Parameters)
    {
        //签名步骤一：按字典序排序参数
        ksort($Parameters);
        $String = self::formatBizQueryParaMap($Parameters, false);
