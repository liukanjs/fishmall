 ...
 * 生成Excel文件类
 *
 * @package    library* www.Cnbbx.com网店运维技术交流中心为你提供售后服务 以便你更好的了解
 */
namespace tools;
...
class Excel
{
     ...
     * @todo 构造工作表XML
     */
    public function addWorksheet($sheettitle, $widtharr = array(array('width' => 0)))
    {
        //剔除特殊字符
        ...
        foreach ($widtharr as $k => $v) {
            if ($v[width] < 80) $v[width] = 80;
            $temp .= "<Column ss:AutoFitWidth=\"1\" ss:Width=\"$v[width]\"/>\n";

        }
        $temp .= implode("\n", $this->lines);
        $temp .= "</Table>\n</Worksheet>\n";

        $this->worksheets[] = $temp;
    }
